...
class Cleaner:
    ...
    def show_time_gaps(self, start_date, end_date, time_df):
      ...
      for index in range(len(time_df)-1):
          ...
          if end_time_before != start_time_after and project_before != "Location" and project_after != "Location":
              disparity = helper.clock_to_seconds(start_time_after)-helper.clock_to_seconds(end_time_before)
      ...
      for index in range(len(time_df)):
          ...
          if start_date_before != end_date_before and end_date_before_time[:5] != '00:00':
              print('Over MIDNIGHT')
    ...
    def show_duplicates(self, start_date, end_date, time_df):
      ...
      for index in range(len(time_df)-1):
          ...
          if desc_before != '' and desc_before == desc_after and project_after != 'Eating' and tag_before == tag_after and start_date_before == start_date_after:
              print(desc_before)
              display(time_df[index:index+2])
      print("Done")
      return None
          ...
          for index in time_df.index:
              ...
              if task_seconds > 3600 and project not in neg_dic:
                  if project in productive:
                      ...
                      if task_seconds/3600 < wasted[project] and task_seconds/3600 > 1: 
                          print("Non Wasted:", project, timedelta(seconds=task_seconds))
                          non_wasted_list.append(task_seconds)
                      if task_seconds/3600 > wasted[project]:
                          print("Non Wasted:", project, timedelta(seconds=wasted[project]*3600))
                          non_wasted_list.append(wasted[project]*3600)
                                                    
                      if (task_seconds-wasted[project]*3600) > 1:
                          print("WASTED:", project, timedelta(seconds=task_seconds-wasted[project]*3600))
